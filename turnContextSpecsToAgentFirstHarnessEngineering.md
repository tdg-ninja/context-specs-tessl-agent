...
# (HUMAN_REVIEW) terminal states — the substrate /evaluate-sessions reads.
...
decision they disagree with. This is also the substrate the
`/evaluate-sessions` skill consumes (built — `.claude/skills/evaluate/evaluate-sessions/`).
...
- **The Human Loop is partially built.** Understanding → Intent → Evaluate (see "The Human Loop"). `/intent` (Intent) and **both** Evaluate skills — `/evaluate-pr` (the change) and `/evaluate-sessions` (the build trail) — are built; one piece remains planned: the **Understanding** phase — an LLM-as-tutor wrapper over the project wiki/knowledge base (the Karpathy LLM-wiki pattern) that builds the human's model before Intent. `/evaluate-sessions` (built — `.claude/skills/evaluate/evaluate-sessions/`) is a sibling of `/evaluate-pr` under the `evaluate/` category that evaluates the `claude -p` build trail (the session IDs posted on every PR by `signal_stuck`, `signal_human_review`, and `signal_learn_review`), so a human resolving a STUCK — or simply curious — can trace which context shaped a decision without hand-reading raw `.jsonl`. Its outcomes are **evals** (regression tests over the harness's own skills/context, under `evals/`) and **context fixes**, landed on a branch (the PR's, or a fresh `capture/<slug>` branch when the PR is discarded) and absorbed into memory via merge + `/learn` P7. Owner: Rico.
...
- **Harness host-worktree sync — resolved.** The host worktree sits at a detached HEAD and does not auto-advance when `main` moves, yet the dispatcher, `bootstrap-worktree.sh`, `.harness/env`, the `/learn` skill, the memory, and the root `AGENTS.md` are all read *from* it. Without a sync step, loop-level infrastructure would freeze at the commit the host worktree was created on. Fix: the outer loop targets a thin wrapper `scripts/harness-tick.sh` that force-syncs the host to a clean `origin/main` (`git checkout -f --detach origin/main` + `git clean -fd`) and then `exec`s the (HEAD-agnostic) dispatcher. The sync lives in the wrapper, before `exec`, so the dispatcher never overwrites its own running file (its `flock` is on `$0`). Feature *pipeline* skills need no sync — they ride per-feature worktrees branched from the PRD branch, so new features pick up updates on their own; only loop infra and `/learn`-time context ride the host worktree. So "merge to main" is the single propagation surface for loop infra too, consistent with the rest of the design. (One caveat the next entry covers: this sync must not run while a long-running `/learn` is mid-flight in the host worktree — guarded by `.harness/learn-running`.)
- **`/learn` runs in the host worktree, not a dedicated one — the right call, but it required a concurrency guard.** `/learn` is dispatched with cwd `"."` (the host worktree) rather than getting its own throwaway worktree like the pipeline steps. Reusing the host worktree is still the correct choice — `harness-tick.sh` already makes `"."` a clean `origin/main` checkout each tick, exactly the state `/learn` needs, so a dedicated `/learn` worktree stays unnecessary. **But dogfooding surfaced a race the original rationale missed:** `/learn` can run *longer than the tick interval* (a from-scratch Expert bootstrap is minutes), and the *next* tick's force-sync (`git checkout -f` + `git clean`) would fire while `/learn` was still writing — wiping its in-flight Expert and `learn/<sha>` branch, so it never reached its push+PR step. Worse, because that tick never completes, `.harness/last-main-sha` never advances and the dispatcher re-triggers `/learn` from scratch every tick: build → wipe → rebuild, with no PR ever landing. Fix: the dispatcher records its PID in `.harness/learn-running` around the `/learn` call, and `harness-tick.sh` skips the sync (exits the tick) while that PID is live, reaping a stale marker from a crashed run. This extends the dispatcher's `flock` serialization to the one bit of per-tick work that lives in the wrapper, outside the lock. So the design decision holds; what changed is that the host-worktree sync is now serialized against a running `/learn`.
